...
This REP proposes a new `ros2log` command-line package for ROS 2 that provides a unified, user-friendly interface for runtime logging configuration and inspection.
The `ros2 log` command will enable users to list, get, set, and monitor logging levels for ROS 2 nodes, improving the debugging and diagnostic capabilities of ROS 2 systems.
...
Currently, logging configuration in ROS 2 from the command line is limited and inconsistent.
...
As ROS 2 logging is crucial for debugging and production diagnostics, a dedicated, ergonomic CLI is necessary to improve developer productivity and system maintainability.
A unified `ros2 log` command would provide:

- Consistent CLI user experience for logging across ROS 2
...
- Alignment with existing ROS 2 CLI patterns
...
Lists all nodes in the ROS 2 graph that have the logger services enabled (i.e., nodes with `enable_logger_service` set to `true`).
...
Displays all valid log severity levels supported by ROS 2.
...
The `ros2 log` command will be implemented as a new package `ros2log` in the `ros2cli` repository, following the existing pattern of other CLI packages:
...
3. **New `ros2 log` command group** (chosen approach):
   - Follows the established pattern in ROS 2 CLI (similar to `ros2 lifecycle`, `ros2 topic`, etc.)
...
The requirement that nodes must enable `enable_logger_service` is maintained from the existing ROS 2 design.
...
- No changes to the ROS 2 logging infrastructure or APIs are required
- The new commands are purely additive to the ROS 2 CLI
...
New users learning ROS 2 will benefit from the intuitive command structure:
...
1. **ROS 2 Tutorials**: Update the "Logging and logger configuration" tutorial to demonstrate `ros2 log` commands
...
To support the `ros2 log list` and `ros2 log describe` commands effectively, the following enhancements are needed in the ROS 2 logging infrastructure.
...
- [ROS 2 Logging Documentation](https://docs.ros.org/en/rolling/Tutorials/Demos/Logging-and-logger-configuration.html)
- [ros2log new 1st class command group support](https://github.com/ros2/ros2cli/issues/1148)
- [Add verb for setting a node's log level](https://github.com/ros2/ros2cli/pull/1060)
